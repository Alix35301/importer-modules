...
data = pd.read_excel(file_path, header=None, sheet_name="Table 1")
df = pd.DataFrame(data)

classification_xls = pd.ExcelFile(r'\\admma2\data\SSD\SIS\FSI\classification.xlsx')
cls = pd.read_excel(classification_xls, sheet_name="FSID", header=None)
df_cls = pd.DataFrame(cls)

codes_from_classification = df_cls.iloc[:, 4].to_list()
unit = [str(unit) for unit in df.iloc[:160, 6]]
...
dates_row = [row for row in df.iloc[10].values.tolist() if str(row) != "nan"]
date_full = [row for row in df.iloc[10].values.tolist()]
for i, row in df.iterrows():
        ...
        if str(code) != "nan" and code in codes_from_classification:
            ...
            for i, j in enumerate(row.tolist()):
                if i > 6 and i <= df.iloc[10].values.tolist().index(str(dates_row[-1])):
                    year = date_full[i][:4]
                    date = ""
...
tk_sql = "Delete from [FSI].[dbo].[F_FSID];DBCC CHECKIDENT ('[FSI].[dbo].[F_FSID]', RESEED, 0)"
...
for i, row in dbdf.iterrows():
    sql = "INSERT INTO [FSI].[dbo].F_FSID (" + cols + ") VALUES (" + "?," * (len(row) - 1) + "?)"
