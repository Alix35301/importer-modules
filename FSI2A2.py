...
data = pd.read_excel(file_path, header=None, sheet_name="Annex 2")
df = pd.DataFrame(data)

classification_xls = pd.ExcelFile(r'\\admma2\data\SSD\SIS\FSI\classification.xlsx')
cls = pd.read_excel(classification_xls, sheet_name="INCOME_EXPENSE", header=None)
...
tk_sql = "Delete from [FSI].[dbo].[F_FSI2A2];DBCC CHECKIDENT ('[FSI].[dbo].[F_FSI2A2]', RESEED, 0)"
...
for i, row in dbdf.iterrows():
    sql = "INSERT INTO [FSI].[dbo].F_FSI2A2 (" + cols + ") VALUES (" + "?," * (len(row) - 1) + "?)"
