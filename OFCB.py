...
data = pd.read_excel(file_path, header=None, sheet_name="BS")
df = pd.DataFrame(data)

classification_xls = pd.ExcelFile(r'\\admma2\data\SSD\SIS\FSI\classification.xlsx')
cls = pd.read_excel(classification_xls, sheet_name="OFC", header=None)
...
tk_sql = "Delete from [FSI].[dbo].[F_OFCB];DBCC CHECKIDENT ('[FSI].[dbo].[F_OFCB]', RESEED, 0)"
...
for i, row in dbdf.iterrows():
    sql = "INSERT INTO [FSI].[dbo].F_OFCB (" + cols + ") VALUES (" + "?," * (len(row) - 1) + "?)"
